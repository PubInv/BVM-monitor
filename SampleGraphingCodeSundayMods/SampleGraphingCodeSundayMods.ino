...
#define TARGET_BAR_PERCENT 0.645 // we'll draw the target line at this percentage of the height
#define GOOD_BAR_PERCENT 0.1785 // 10% below the target percentage
#define BAD_BAR_PERCENT 0.355 // 10% above the target percentage
...
//  Coloring Background, blue - too little, red - too much, green - good
...
//Colored Version
// ALOIS, this looks pretty good to me (I can't test it), but I think you want fillNotDraw to be true. I have
// changed your code below.
 if (mheight <= theight)
 {
  myDrawRect(left,SCREEN_HEIGHT - mheight,width, SCREEN_HEIGHT,BLUE, true);
 display.drawLine(left,SCREEN_HEIGHT - mheight, left+ width-1, SCREEN_HEIGHT - mheight, BLUE);
 }
 else if (mheight >= gheight)
 {
  myDrawRect(left,SCREEN_HEIGHT - mheight,width, SCREEN_HEIGHT,RED, true);
  display.drawLine(left,SCREEN_HEIGHT - mheight, left+ width-1, SCREEN_HEIGHT - mheight, RED);
 }
 else
 {
  myDrawRect(left,SCREEN_HEIGHT - mheight,width, SCREEN_HEIGHT,GREEN, true);
 ...
 // so we read the millisecond clock again...
  ...
  //  render_empty_bars();
    if (G_volume_previous != G_volume) {
        ...
        // ALOIS, I suggest you add this line here, and draw the target line AFTER you have render the other bars
